...
function formatIndexForPrompt(index: ExerciseIndex): string {
  ...
  Object.entries(index).forEach(([bodyPart, data]) => {
    ...
    grouped[mainPart].push(...data.exercises);
  });

  // Format output with ALL exercises
  Object.entries(grouped).forEach(([bodyPart, exercises]) => {
    const uniqueExercises = [...new Set(exercises)];
    output += `**${bodyPart}** (${uniqueExercises.length}):\n`;
  ...
  output += `- Format exercise names with double brackets: [[Exercise Name]]\n`;
...
if (require.main === module) {
  ...
  // Generate formatted indexes for each language
...
export { generateIndex, formatIndexForPrompt };
