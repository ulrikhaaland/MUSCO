...
export function ExerciseDetailModal({
  ...
  return (
    <>
      ...
      <div 
        className="hidden md:flex fixed inset-0 z-[80] bg-black/80 items-center justify-center p-4"
        onClick={onClose}
      >
        <div 
      ...
      {/* Mobile: Full-Page View */}
      <div className="md:hidden fixed inset-0 z-[80] bg-gray-900 flex flex-col">
        ...
        <div className="sticky top-0 bg-gray-900 border-b border-gray-700 px-4 py-3 flex items-center gap-3 z-10">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-white font-semibold text-base truncate">{exercise.name}</h3>
            {exercise.bodyPart && (
              <span className="inline-block mt-0.5 px-2 py-0.5 rounded bg-indigo-600/20 text-indigo-300 text-xs border border-indigo-500/40">
                {exercise.bodyPart}
              </span>
            )}
          </div>
        ...
        {/* Fixed Footer - Video Button */}
        {exercise.videoUrl && (
          <div className="sticky bottom-0 border-t border-gray-800 p-4 bg-gray-900 safe-area-bottom">
            <button
              onClick={() => onVideoClick(exercise)}
              disabled={loadingVideo}
