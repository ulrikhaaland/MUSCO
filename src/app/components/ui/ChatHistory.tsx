...
interface ChatHistoryProps {
  ...
  titleGeneratingForChatId?: string | null;
}
...
export function ChatHistory({
  ...
  // Load chat history when opened or refreshTrigger changes
  useEffect(() => {
    if (!user?.uid) return;
    // Only load if open, or if refresh trigger changed (to update list in background)
    if (!isOpen && refreshTrigger === undefined) return;
  ...
  }, [isOpen, user?.uid, refreshTrigger]);
  ...
  const confirmDelete = async () => {
    if (!user?.uid || !chatToDelete) return;

    try {
      await deleteChatSession(user.uid, chatToDelete);
  ...
  return (
    <Drawer isOpen={isOpen} onClose={onClose} side="right" width="w-80">
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      {titleGeneratingForChatId === chat.id ? (
      ...
      <ConfirmationDialog
        ...
        description={t('chatHistory.deleteDialog.description')}
        confirmText={t('chatHistory.deleteDialog.confirm')}
        cancelText={t('chatHistory.deleteDialog.cancel')}
